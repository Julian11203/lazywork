...
function insertarInicio() {
    ...
    let tiempodesesion = $("#tiempodesesion").val();
    ...
    $.ajax({
        ...
        contentType: "application/json",
        success: function () {
            // Manejar el éxito, por ejemplo, cerrar el modal y actualizar la lista
            $("#usuarioID").val('');
            $("#fechaHoraFin").val('');
            $("#fechaHoraInicio").val('');
            $("#tiempodesesion").val('');
            $("#nombreuser").val('');
            $('#registrarModal5').modal('hide');
            listarInicios(); // Asumiendo que tienes una función para listar los inicios
        },
        error: function (xhr) {
...
// Otras funciones y código relacionado...

function buscarUsuarioPorId() {
    // Agrega tu lógica para buscar al usuario por ID
    console.log("Buscando al usuario por ID...");
    // Puedes realizar una solicitud AJAX u otra lógica aquí
}
        ...
        .then(data => {
            ...
            data.forEach(inicioSesion => {
                ...
                let row = `<tr>
                                ...
                                <td>${inicioSesion.fechaHoraInicio}</td>
                                <td>${inicioSesion.fechaHoraFin}</td>
                                <td>${inicioSesion.tiempodesesion}</td>
                                <td>${inicioSesion.usuarioback}</td>
                                <td>${inicioSesion.nombreuser}</td>
                                <td>
